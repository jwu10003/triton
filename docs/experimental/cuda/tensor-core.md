...
| 数据类型 | Volta 7.0 | Turing 7.5 | Ampere 8.0 | Hopper 9.0 | Blackwell 10.0 | Blackwell 12.0 |
|----------|:---------:|:----------:|:----------:|:----------:|:--------------:|:--------------:|
| FP16 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| BF16 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| TF32 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
...
*Blackwell 的 FP64 Tensor Core 吞吐较低，非主要使用场景。SM 12.0 (RTX 5090) 不支持 FP64 Tensor Core。
...
| 规格 | V100 SXM2 | A100 SXM | H100 SXM5 | B200 | RTX 5090 |
|------|-----------|----------|-----------|------|----------|
| **架构** | Volta | Ampere | Hopper | Blackwell | Blackwell (消费级) |
| **工艺** | 12nm | 7nm | 4nm | 4nm 双芯 | 4nm |
| **SM 数** | 80 | 108 | 132 | 148 | 170 |
| **TC/SM** | 8 | 4 | 4 | 4 | 4 |
| **TC 代次** | 1st | 3rd | 4th | 5th | 5th (mma.sync) |
| **FP16 TC TFLOPS** | 125 | 312 | ~990 | ~2,250 | ~210† |
| **TF32 TC TFLOPS** | — | 156 | ~495 | ~1,125 | ~105† |
| **FP8 TC TFLOPS** | — | — | ~1,979 | ~4,500 | ~419† |
| **FP4 TC TFLOPS** | — | — | — | ~9,000 | ~1,676 |
| **INT8 TC TOPS** | — | 624 | ~1,979 | ~4,500 | ~838 |
| **显存容量** | 32 GB HBM2 | 80 GB HBM2e | 80 GB HBM3 | 180 GB HBM3e | 32 GB GDDR7 |
| **显存带宽** | 900 GB/s | 2.0 TB/s | 3.35 TB/s | 8.0 TB/s | 1.79 TB/s |
| **L2 Cache** | 6 MB | 40 MB | 50 MB | 126 MB | 96 MB |
| **Shared Mem/SM** | 96 KB | 164 KB | 228 KB | 228 KB | 128 KB |
| **TDP** | 300W | 400W | 700W | 1000W | 575W |

> †RTX 5090 TC TFLOPS 为 FP32 累加器模式。消费级 Blackwell 的 FP32 累加吞吐为半速；使用 FP16 累加器时吞吐翻倍 (如 FP16 TC 可达 ~419 TFLOPS)。
...
| 设计维度 | Ampere (SM 8.x) | Hopper (SM 9.0) | Blackwell DC (SM 10.0) | Blackwell 消费级 (SM 12.0) |
|---------|----------------|----------------|----------------------|--------------------------|
| Thread Block 大小 | 128–256 | 128 (Warpgroup) | 128–256 | 128–256 |
| 数据搬运 | `cp.async` + 双缓冲 | TMA + `mbarrier` | TMA + `mbarrier` | TMA / `cp.async` |
| MMA 执行 | 同步 (mma.sync) | 异步 (wgmma) | 异步 (tcgen05) | 同步 (mma.sync 扩展) |
| 累加器存储 | 寄存器 | 寄存器 | TMEM | 寄存器 |
| Shared Mem 布局 | Swizzle / Padding | TMA Swizzle (128B) | TMA Swizzle | Swizzle / TMA Swizzle |
| 最大 SMEM/SM | 164 KB | 228 KB | 228 KB | 128 KB |
| 最大 MMA Tile | m16n8k16 | m64n256k16 | m256n256k16 (2SM) | m16n8k16 |
| Warp 专化 | 可选 | 推荐 (Producer/Consumer) | 推荐 | 可选 |
