...
```
场景: 每线程需处理 16B 数据 (如 4 个 float)

标量加载:   每线程每次 4B (LDG.E.32)   → 需 4 条指令/线程 → Warp 发射 4 条指令
向量化加载: 每线程每次 16B (LDG.E.128) → 需 1 条指令/线程 → Warp 发射 1 条指令

收益:
- Warp 级指令数减少 4× (1 条 LDG.E.128 替代 4 条 LDG.E.32)
- 每条指令传输更多数据 (512B vs 128B per warp)
- 更少的指令发射开销和调度压力
```
