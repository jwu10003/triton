...
| 组件 | Volta 7.0 | Turing 7.5 | Ampere 8.0 | Ampere 8.6 | Ada 8.9 | Hopper 9.0 | Blackwell 12.0 |
|------|----------|----------|----------|----------|---------|----------|--------------|
| FP32 单元 | 16 | 16 | 16 | 32* | 32* | 32 | 32† |
| INT32 单元 | 16 | 16 | 16 | 16* | 16* | 16 | 32† |
| FP64 单元 | 8 | ~0 | 8 | ~0 | ~0 | 16 | ~0 |
| SFU | 4 | 4 | 4 | 4 | 4 | 4 | 4 |
| LD/ST 单元 | 8 | 4 | 8 | 8 | 8 | 8 | 8 |
| Tensor Core | 2 | 2 | 1 | 1 | 1 | 1 | 1 |
| Warp Scheduler | 1 | 1 | 1 | 1 | 1 | 1 | 1 |
| Register File | 64 KB | 64 KB | 64 KB | 64 KB | 64 KB | 64 KB | 64 KB |

> *CC 8.6/8.9: 第二通路中 16 个 FP32/INT32 双用单元，可执行 FP32 或 INT32 但不能同时执行。
> †CC 12.0: 32 个 FP32/INT32 统一 ALU，可执行 FP32 或 INT32 但不能同时执行 (非双发射)。
...
| Blackwell (消费级) | 12.0 | 128 | 128 | FP32 或 INT32 | 统一 ALU, 不可并发 |
...
| 指令类型 | CC 7.0 | CC 7.5 | CC 8.0 | CC 8.6/8.9 | CC 9.0 | CC 12.0 |
|----------|--------|--------|--------|-----------|--------|---------|
| **FP32 add/mul/fma** | 64 | 64 | 64 | 128 | 128 | 128 |
| **FP64 add/mul/fma** | 32 | 2 | 32 | 2 | 64 | 2 |
| **INT32 add** | 64 | 64 | 64 | 64 | 64 | 128* |
| **FP16×2 (half2)** | 128 | 128 | 128 | 256 | 256 | 256 |
| **BF16×2** | — | — | 128 | 256 | 256 | 256 |
| **SFU** (sin/cos/rsqrt) | 16 | 16 | 16 | 16 | 16 | 16 |
| **类型转换** | 16 | 16 | 32 | 32 | 64 | 32 |
| **LD/ST 单元** | 32 | 16 | 32 | 32 | 32 | 32 |

> *CC 12.0 INT32: 128 个统一 ALU 均可执行 INT32，但与 FP32 不可并发 (同一周期只能选 FP32 或 INT32)。
...
L1 Cache:            │ ~28-33 cyc  ~128 B/clk/SM    L1+SMEM 共│  ← 自动缓存
                     ├────────────────────────────────────┤
L2 Cache:            │ ~200 cycles ~6-12 TB/s       6-126 MB  │  ← 全芯片共享
                     ├────────────────────────────────────┤
Global Memory (HBM): │ 400-800 cyc 0.9-8 TB/s      32-180 GB │  ← 主存
                     └────────────────────────────────────┘
```
...
| L2 Cache | 6 MB | 40 MB | 50 MB | 126 MB |
| HBM 容量 | 32 GB | 80 GB | 80 GB | 180 GB |
...
```
CUDA Core FP32:    128 ops/cycle/SM × 132 SM × 1.98 GHz ≈  33.5 TFLOPS (FMA: ~67 TFLOPS)
Tensor Core FP16:  ~990 TFLOPS
Tensor Core FP8:   ~1,979 TFLOPS

→ Tensor Core FP16 吞吐是 CUDA Core FP32 的 ~15×
→ Tensor Core FP8  吞吐是 CUDA Core FP32 的 ~30×
```
